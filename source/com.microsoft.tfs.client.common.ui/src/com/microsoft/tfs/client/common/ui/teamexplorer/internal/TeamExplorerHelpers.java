...
public class TeamExplorerHelpers {
     ...
     * syntax:
     * 
     * -openuri vsoe://<path>/?url=<git-repository-url>&Ref=master&EncFormat=<encoding>[&<other-parameters>...]
     * 
     * At this moment we use vsoi instead of vsoe, and ignore <path> and
     * <other-parameters>
     * @formatter:on
     */
    public static boolean tryParseProtocolHandlerArguments(final String[] applicationArgs) {
        ...
        for (final String arg : applicationArgs) {
            if (found) {
                log.info(
                    MessageFormat.format("Found the protocol handler argument: {0} {1}", PROTOCOL_HANDLER_ARG, arg)); //$NON-NLS-1$

                final URI uri;
                try {
                    uri = URIUtils.newURI(arg);
                ...
                if (!PROTOCOL_HANDLER_SCHEME.equalsIgnoreCase(uri.getScheme())) {
                    log.error(MessageFormat.format(
                        "   Incorrect scheme in the protocol handler URL: {0}", //$NON-NLS-1$
                        uri.getScheme() == null ? "NULL" : uri.getScheme())); //$NON-NLS-1$
                    found = false;
                    continue;
                }
