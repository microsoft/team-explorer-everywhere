...
/**
 * Result attachment link control provider contains only a label which instructs
 * users how to create this type of link. This provider always returns true for
 * isValid and an empty link array so that the OK button can be pressed to
 * dismiss the dialog without altering any links.
 */
public class ResultAttachmentLinkControlProvider implements LinkControlProvider {
    @Override
    public String getDisplayName(final RegisteredLinkType linkType) {
        return LinkUIRegistry.RESULTATTACHMENT_PROVIDER_DISPLAY_NAME;
    }

    @Override
    public void initialize(final Composite composite) {
        final Label label = new Label(composite, SWT.WRAP);
        label.setText(Messages.getString("ResultAttachmentLinkControlProvider.SummaryText")); //$NON-NLS-1$
