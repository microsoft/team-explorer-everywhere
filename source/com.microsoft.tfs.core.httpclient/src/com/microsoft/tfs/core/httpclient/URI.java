 ...
 * /HTTPCLIENT_3_1/src/java/org/apache/commons/httpclient/URI.java $ $Revision:
 * 564973 $ $Date: 2007-08-11 22:51:47 +0200 (Sat, 11 Aug 2007) $
 *
 * ====================================================================
 *
...
public class URI implements Cloneable, Comparable, Serializable {
    ...
    public URI(final URI base, final URI relative) throws URIException {
        ...
        if (base._scheme != null) {
            ...
            _authority = base._authority;
            _is_net_path = base._is_net_path;
        }
        if (base._is_opaque_part || relative._is_opaque_part) {
            _scheme = base._scheme;
            _is_opaque_part = base._is_opaque_part || relative._is_opaque_part;
            _opaque = relative._opaque;
            _fragment = relative._fragment;
            setURI();
            return;
        }
        ...
        if (relative._scheme != null && (!schemesEqual || relative._authority != null)) {
            _scheme = relative._scheme;
            _is_net_path = relative._is_net_path;
            _authority = relative._authority;
            if (relative._is_server) {
                _is_server = relative._is_server;
                _userinfo = relative._userinfo;
                _host = relative._host;
                _port = relative._port;
            } else if (relative._is_reg_name) {
                _is_reg_name = relative._is_reg_name;
            }
            _is_abs_path = relative._is_abs_path;
            _is_rel_path = relative._is_rel_path;
            _path = relative._path;
        } else if (base._authority != null && relative._scheme == null) {
            _is_net_path = base._is_net_path;
            _authority = base._authority;
            if (base._is_server) {
                _is_server = base._is_server;
                _userinfo = base._userinfo;
                _host = base._host;
                _port = base._port;
            } else if (base._is_reg_name) {
                _is_reg_name = base._is_reg_name;
            }
        }
        ...
        if (relative._authority == null && (relative._scheme == null || schemesEqual)) {
            if ((relative._path == null || relative._path.length == 0) && relative._query == null) {
                ...
                _path = resolvePath(base._path, relative._path);
            }
        }
        // base._query removed
        if (relative._query != null) {
            _query = relative._query;
        }
    ...
    protected void parseUriReference(final String original, final boolean escaped) throws URIException {
        ...
        final int atColon = tmp.indexOf(':');
        final int atSlash = tmp.indexOf('/');
    ...
    public void setRawPath(char[] escapedPath) throws URIException {
        ...
        if (_is_net_path || _is_abs_path) {
            ...
            final int at = indexFirstOf(escapedPath, '/');
            if (at == 0) {
    ...
    protected char[] getRawCurrentHierPath(final char[] path) throws URIException {
        ...
        final int first = buff.indexOf('/');
        final int last = buff.lastIndexOf('/');
        if (last == 0) {
            return rootPath;
    ...
    public String getEscapedAboveHierPath() throws URIException {
        final char[] path = getRawAboveHierPath();
        return (path == null) ? null : new String(path);
    }
    ...
    public String getEscapedName() {
        final char[] basename = getRawName();
    ...
    public String getEscapedPathQuery() {
        final char[] rawPathQuery = getRawPathQuery();
    ...
    protected char[] removeFragmentIdentifier(char[] component) {
        ...
        final int lastIndex = new String(component).indexOf('#');
        if (lastIndex != -1) {
    ...
    protected char[] normalize(final char[] path) throws URIException {
        ...
        while ((index = normalized.indexOf("/../", startIndex)) != -1) {
            final int slashIndex = normalized.lastIndexOf('/', index - 1);
            if (slashIndex >= 0) {
        ...
        if (normalized.endsWith("/..")) {
            final int slashIndex = normalized.lastIndexOf('/', normalized.length() - 4);
            if (slashIndex >= 0) {
    ...
    public char[] getRawURIReference() {
        ...
        final String uriReference = new String(_uri) + "#" + new String(_fragment);
    ...
    public String getEscapedURIReference() {
        final char[] uriReference = getRawURIReference();
    ...
    public java.net.URI toJavaNetUri() throws URISyntaxException {
