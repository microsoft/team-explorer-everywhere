...
public class WizardConnectionOfflinePage extends ExtendedWizardPage {
    public static final String PAGE_NAME = "WizardConnectionOfflinePage"; //$NON-NLS-1$

    public static final String CONNECTION_ONLINE = "repositoryOnline"; //$NON-NLS-1$

    public WizardConnectionOfflinePage() {
        super(
            PAGE_NAME,
            Messages.getString("WizardConnectionOfflinePage.WorkingOffline"), //$NON-NLS-1$
            Messages.getString("WizardConnectionOfflinePage.WorkingOffline")); //$NON-NLS-1$

        setErrorMessage(Messages.getString("WizardConnectionOfflinePage.WorkingOffline")); //$NON-NLS-1$
    }
    ...
    protected void doCreateControl(final Composite parent, final IDialogSettings dialogSettings) {
        ...
        if (actionText == null) {
            actionText = Messages.getString("WizardConnectionOfflinePage.Continuing"); //$NON-NLS-1$
        }

        final Label errorText = new Label(container, SWT.WRAP | SWT.READ_ONLY);

        final String messageFormat = Messages.getString("WizardConnectionOfflinePage.OfflineErrorLabelTextFormat"); //$NON-NLS-1$
    ...
    protected void refresh() {
        ...
        if (projectManager.isConnecting()) {
            getExtendedWizard().removePageData(CONNECTION_ONLINE);
        } else {
            getExtendedWizard().setPageData(CONNECTION_ONLINE, Boolean.TRUE);
        }
    }
}
